...
class MappingController extends AbstractController
{
    ...
    public function ambassadors(Request $request, Features $features, TranslatorInterface $translator)
    {
        ...
        if (!is_null($page)) {
            $response = $client->request('GET', getenv('API_URL') . '/users?expand=reviews&user_filter[step]=3&limit=50'
            );
            $statusCode = $response->getStatusCode();
            if ($statusCode == 200) {
                $users = $response->toArray();
    ...
    public function popupAmb(Request $request, TranslatorInterface $translator)
    {
        ...
        $response = $client->request('GET', getenv('API_URL') . '/users?expand=reviews&user_filter[username]='.$request->get('id')
        );
        $statusCode = $response->getStatusCode();
        if ($statusCode == 200) {
